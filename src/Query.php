...
class Query
{
	...
	)
	{
		...
		$matches = [];
		preg_match_all(
			'/\b(DROP EVENT|DROP EVENT IF EXISTS|TRUNCATE|TRUNCATE TABLE|CREATE TABLE IF NOT EXISTS|CREATE TABLE|DROP TABLE IF EXISTS|DROP TABLE|ALTER TABLE|INSERT IGNORE INTO|INSERT INTO|INSERT|UPDATE|SELECT .* FROM|JOIN|DELETE FROM|DELETE IGNORE FROM|CREATE INDEX .+ ON)\s+([`]*[a-zA-Z0-9_.$]+[`]*)/im',
			$this->getContent(),
			$matches
		);
