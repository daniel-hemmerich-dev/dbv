...
class Deploy
{
     ...
     */
    public function setWhitelistSync(array $whitelistSync)
    {
     ...
     * @return array
     */
    protected function readStatementsFromFile(): array
    {
        $file = fopen(realpath($this->getPostScript()), "r");
        $stmts = [];
        $stmt = "";
        while (!feof($file)) {
            $line = fgets($file);
            if (preg_match("/^\n/", $line)) {
                //line begins with newline
                if (!empty($stmt)) {
                    $stmts[] = $stmt;
                    $stmt = "";
                }
